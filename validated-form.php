...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (in_array($raw_name, $existing_users, false)) {
  ...
  if(in_array($raw_character, ["wizard", "mage", "orc"])) {
    ...
    $validation_error = $validation_error . "You must pick a wizard, mage, or orc. <br>";
  }
  ...
  if (filter_var($raw_email, FILTER_VALIDATE_EMAIL)){ 
    ...
    $validation_error = $validation_error . "Invalid email. <br>";
  }
  $raw_birth_year = $_POST["birth_year"];
  $options = ["options" => ["min_range" => 1901, "max_range" => date("Y")]];
  if (filter_var($raw_birth_year, FILTER_VALIDATE_INT, $options)) {
    $birth_year = $raw_birth_year;
  } else {
    $validation_error = $validation_error . "That can't be your birth year. <br>";
  }
